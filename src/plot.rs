...
) -> Result<(), Box<dyn std::error::Error>>
{
    ...
    chart.draw_series(
            SurfaceSeries::xoz(
                ...
                let points = polygon.point_iter();

                let z_tot: T = points.into_iter()
                    .map(|&(_x, z, _y)| {
                        z
                    }).sum();

                let z_avg = z_tot/(points.len() as T);

                let c = ((z_avg - z_min)/(z_max - z_min)*(u8::MAX as T + 1.0 - T::EPSILON)) as u8;

                Polygon::new(points, RGBColor(u8::MAX, u8::MAX - c, 0).mix(0.2).filled())
            }),
        )?
        .label("Surface")
    ...
    chart.draw_series(
            ...
            .map(|polygon| {
                ...
                let points: Vec<(T, T, T)> = polygon.point_iter()
                    ...
                    .collect();
                let avg_theta = if exception {core::f64::consts::PI} else {sum_theta / n_theta as T};
