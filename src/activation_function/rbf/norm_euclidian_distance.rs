    ...
    <T as NeuronStateDerivateable<f64>>::Slope: Clone
{
    ...
    fn slope_params(self: &Self, x: f64) -> Vec<<T as NeuronStateDerivateable<f64>>::Slope>
    {
        if !x.is_zero()
        {
            let mu: Vec<<T as NeuronStateDerivateable<f64>>::Slope> = self.d.iter()
                .flatten()
                .map(|&d| (-d/x).max(f64::MIN).min(f64::MAX).into_state())
                .collect();
            let sigma: Vec<<T as NeuronStateDerivateable<f64>>::Slope> = self.d.iter()
                .flatten()
                .map(|&d| (-0.5*d*d/x).max(f64::MIN).min(f64::MAX).into_state())
                .collect();
            [mu, sigma].concat()
        }
        else
        {
            let mu: Vec<<T as NeuronStateDerivateable<f64>>::Slope> = self.mu_sigma.iter()
                .flatten()
                .map(|&(_, sigma)| (-sigma.abs().sqrt()/sigma).max(f64::MIN).min(f64::MAX).into_state())
                .collect();
            let sigma: Vec<<T as NeuronStateDerivateable<f64>>::Slope> = mu.iter()
                .map(|_| 0.0.into_state())
                .collect();
            [mu, sigma].concat()
        }
    }
    fn accumulate(self: &mut Self, inputs: &[Vec<f64>], bias: f64) -> Option<f64>
    {
        ...
        Some((dist2.abs().sqrt()*dist2.signum()).into_state())
    }
}