...
pub struct BoundingFunctionEuclidianDistance
{
    mu: Vec<Vec<f64>>,
    #[serde(skip)]
    d: Vec<Vec<f64>>,
...
impl BoundingFunctionEuclidianDistance
{
    pub fn new<
    ...
    where Self: Into<O>
    {
        ...
        BoundingFunctionEuclidianDistance {
            mu: SIZES.iter()
                .map(|size| (0..*size)
...
#[typetag::serde(name = "EuclidianDistance")]
#[clone_box]
impl BoundingFunctionAny for BoundingFunctionEuclidianDistance
{
    
}

#[clone_box]
impl<T> BoundingFunction<f64, T> for BoundingFunctionEuclidianDistance
where
